...
export async function checkIfFriends(player_id:number, potential_friend_id:number, dbi:WordleDBI) {
    const friends =  dbi.db().get("friends.plr#" + player_id);
    var filteredFriendList = await friends.find({id:potential_friend_id});
    console.log("Player " + player_id + " potential friend " + potential_friend_id + " on list " + filteredFriendList)
