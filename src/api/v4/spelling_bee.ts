...
spelling_bee.post('/season_info',async (req, res, next) => {
    var season_rules = await getSeasonRules();
    res.json({season_id: season_rules.id, season_title: season_rules.season_title, rules:season_rules.rules, points_rules: season_rules.points, seconds_to_end:season_rules.getSecondsToEnd()})
})
